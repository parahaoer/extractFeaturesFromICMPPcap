...
class FeatureExtractor():
    ...
    def extractFeaturesWithMultiprocess(self, ip_pair_datas, features, is_negative_sample):
        ...
        distance_in_ICMP_pair_list = levenshteinDistanceCalculator.getDistanceInICMPPair(ip_pair_datas)
        ...
        distance_between_type_8_list = levenshteinDistanceCalculator.getDistanceBetweenSameside(ip_pair_datas, 8)
        ...
        distance_between_type_0_list = levenshteinDistanceCalculator.getDistanceBetweenSameside(ip_pair_datas, 0)
        ...
        if is_negative_sample.value == True:
            feature_vec.append(1)
        elif is_negative_sample.value == False:
    ...
    def extractFeaturesFromIPPairs(self):
        ...
        is_negative_sample = manager.Value('is_negative_sample', False)
        ...
        for ip_key in ip_pairs_dict.keys():
            ...
            if(ip_key[0].find('negative') == 0):
                is_negative_sample.value = True
            elif(ip_key[0].find('positive') == 0):
                is_negative_sample.value = False
