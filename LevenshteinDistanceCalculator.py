...
class LevenshteinDistanceCalculator():
    ...
    def getDistanceInICMPPair(self, ip_pair_datas):
            ...
            for icmp_packet in icmp_packet_list:
                ...
                if request_icmp_payload != b'' and reply_icmp_payload != b'':
                    distance_in_ICMP_pair += self.normal_leven(
                        request_icmp_payload, reply_icmp_payload)
    ...
    def getDistanceBetweenSameside(self, ip_pair_datas, type_code):
            ...
            try:
                ...
                if icmp_type == type_code:
                    ...
                    if prev != b'' and packet_payload != b'':
                        ...
                        distance_between_same_side = self.normal_leven(
                            prev, packet_payload)
                        distance_between_same_side_list.append(
                            distance_between_same_side)

            except KeyError as e:
                print('src_ip=' + ip_packet.fields['src'])
                print('data_len=' + str(len(data.original)))
                print('icmp_packet_len=' + str(len(icmp_packet.original)))
                print('icmp_packet_hex=' + icmp_packet.original.hex())
                print(e)
                continue
